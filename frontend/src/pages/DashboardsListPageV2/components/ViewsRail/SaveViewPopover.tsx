...
// Save-as-view form: name + icon picker. Captures the current filter state into
// a named view (the caller wires `onSave` to the active-view hook).
function SaveViewPopover({
	...
	// Reset the draft each time the popover opens.
	useEffect(() => {
		if (open) {
			setName('');
